...
class SQLiteTest extends PHPUnit_Framework_TestCase {
    ...
    public function coreTests() {
        ...
        $orm = ORMFactory::load(AgilePHP::getWebRoot() . '/orm_mssql_test.xml');
        ...
        $role->persist();  // must persist first since cascade="none" in orm.xml
        $role->setId(ORM::getLastInsertId());  // manually set the id
        ...
        $role2->persist();
        $role2->setId(ORM::getLastInsertId());

        $user2->setRole($role2);
        $user2->merge();

        // tests looking up the user using "restrictions" suite
        $orm->setRestrictions(array('username' => 'phpunit'));
        ...
        PHPUnit_Framework_Assert::assertEquals('agilephp_test_mssql', $Database->getName(), 'Failed to reverse engineer database name');
        PHPUnit_Framework_Assert::assertEquals('mssql', $Database->getType(), 'Failed to reverse engineer database type');
        PHPUnit_Framework_Assert::assertInternalType('array', $tables, 'Failed to reverse engineer database tables');
        ...
        $user1 = new User('sproc', $digest, 'sproc@mssql', '04/13/10', false, 1);
        $user2 = new User('sproc2', $digest, 'sproc@mssql', '04/13/10', false, 1);
        $orm->persist($user1);
        $orm->persist($user2);

        $orm->query('CREATE PROCEDURE [dbo].[authenticate](@userid varchar(150), @passwd varchar(255)) AS SELECT enabled as authenticate FROM users WHERE username = @userid AND password = @passwd;');
        $orm->query('CREATE PROCEDURE [dbo].[getusers] AS SELECT * from users;');
        ...
        PHPUnit_Framework_Assert::assertEquals(1, $auth->getResult(), 'Failed to get expected \'authenticate\' stored procedure result.');

        $getusers = new SPusers();
        $users = $orm->call($getusers);

        PHPUnit_Framework_Assert::assertInternalType('array', $users, 'Failed to get expected \'getusers\' stored procedure result.');
    ...
    public function testGetPrimaryKeyColumns() {
        ...
        PHPUnit_Framework_Assert::assertEquals('id', $columns[0]->getName(), 'Failed to locate \'id\'.');
    }
    ...
    public function testGetForeignKeys() {
        ...
        PHPUnit_Framework_Assert::assertEquals('id', $fKey->getReferencedColumn(), 'testGetForeignKeys failed to get referenced column \'id\'');
    }
