...
class MySQLTest extends PHPUnit_Framework_TestCase {
    ...
    public function mysqlTests() {
        ...
        //$role->persist();  not needed since foreign key configured using cascade="all"

        $user->setRole($role);
        $user->persist();  // saves both User and Role ActiveRecord's

        // tests looking up the user using the model state to define search criteria
        $result = $orm->find(new User('phpunit'));
        $phpunit = $result[0];

        PHPUnit_Framework_Assert::assertEquals('phpunit', $phpunit->getUsername(), 'Failed to persist new user');
