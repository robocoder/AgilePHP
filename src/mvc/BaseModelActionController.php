...
abstract class BaseModelActionController extends BaseModelXslController {
    ...
    protected function setPrimaryKeys($ids) {
        ...
        if(strpos($ids, '_') === false) {
            ...
            $mutator = ClassUtils::toMutator($pkeyColumns[0]->getModelPropertyName());
        ...
        for($i=0; $i<count($idz); $i++) {
            ...
            $mutator = ClassUtils::toMutator($pkeyColumns[$i]->getModelPropertyName());
            ...
            if($pkeyColumns[$i]->isForeignKey()) {
                ...
                $fModelMutator = ClassUtils::toMutator($pkeyColumns[$i]->getForeignKey()->getReferencedColumnInstance()->getModelPropertyName());
    ...
    protected function setModelValues() {
        ...
        foreach($table->getColumns() as $column) {
            ...
            $accessor = ClassUtils::toAccessor($name);
            $mutator = ClassUtils::toMutator($name);
            ...
            if($column->isForeignKey()) {
                ...
                $refMutator = ClassUtils::toMutator($column->getForeignKey()->getReferencedColumnInstance()->getModelPropertyName());
