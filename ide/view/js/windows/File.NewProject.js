AgilePHP.IDE.Window.File.NewProject = function() {
	...
	AgilePHP.IDE.getConfig( 'workspace', function( config ) {
			...
			Ext.getCmp( id + '-form-workspace' ).setValue( config.value );
	});
	...
	return window.wizard([{
              		...
              		handler: function() {
              			...
              			AgilePHP.IDE.FileExplorer.tree.getNodeById( '.:' ).reload(); // Reload tree root (workspace)
              			Ext.getCmp( id ).close();
              		}
              	}
      ]);
